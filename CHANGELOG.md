...
- **`bin/roadmap-renumber --map old=new,…`.** One atomic pass across ROADMAP/TODOS/PROGRESS/designs/archive. Digit/letter lookarounds so `Group 147_` italics match. Dated-historical mentions (`split from the 2026-08-14 103A`, `the retired 104C`) stay put. `--dry-run` and `--map-file` supported.
...
**Added: per-skill preamble + epilogue blocks** — five extend skill files (`skills/{full-review,pair-review,review-apparatus,test-plan,roadmap}.md`) each gain two canonical bash blocks wrapped in `<!-- SHARED:telemetry-preamble -->` and `<!-- SHARED:telemetry-epilogue -->` markers. The preamble reads the telemetry tier via `gstack-config get telemetry` (silent fallback to `off` if missing) and writes the start line to `~/.gstack/analytics/skill-usage.jsonl`. The epilogue computes duration and calls the wrapper with `--skill extend:<name> --duration N --outcome X`. Session id is `$$-$(date +%s)-$RANDOM` so two skills in the same bash subshell within the same second can't collide. Only the `_GE_SKILL="extend:<name>"` line differs per skill; everything else is byte-identical across the cohort.

**Note: no pending-marker crash detection.** The Track 13A design considered writing `.pending-$SESSION_ID` markers for cross-session crash visibility, but the adversarial review caught a phantom-row class of bugs: when gstack's `gstack-telemetry-log` finalize loop sees an extend skill's still-in-flight marker (concurrent multi-session Claude Code, scripts running alongside CC), it would write a duplicate `outcome:unknown` row that conflicts with the extend epilogue's real `outcome:success` row. Crash detection across both ecosystems requires coordinated namespacing (`.pending-extend-*` plus a gstack-proper patch) and is deferred to a future cross-repo Track.
...
Manual testing sessions just got materially less repetitive. When you generate a test plan in `/pair-review`, the agent now infers optional `Covers: [N, M]` links between items: if signing in covers items 3 (session cookie set) and 4 (logged-in nav renders), those links surface at plan-review time so you can vet them before any testing starts. Then in Phase 2, when you pass item 1, instead of clicking through items 3 and 4 individually, you see one bundled prompt: "Item 1 passed. Items 3, 4 are flagged as verified by your action. All pass, mark individually, or park a bug?" A 5-item plan resolves in 2-3 prompts instead of 5; on tightly coupled groups the throughput delta is 2-3x. The trust posture is calibrated: "verified by action" means your action plausibly demonstrated the property — when the property requires direct inspection (logs, cookies, DB state), the bundle prompt tells you to mark individually and verify each. Same posture as the existing BATCH mode, which already accepts "All pass" across three items.

Integrity isn't a casualty of efficiency. A new `PASSED_BY_COVERAGE` status sits alongside `PASSED` so the FAIL handler can distinguish bundled confirmations from direct evidence. When a covering item later fails — on ANY FAIL path, not just Fix Now — the items it backed get pulled back to `UNTESTED` for re-verification. Directly-`PASSED` items never demote (they have independent evidence and stay passed). Multi-cover provenance is tracked: if item 5 is covered by both items 1 and 2, item 5 only demotes when BOTH 1 and 2 are non-passed. Transitive coverage is explicitly NOT supported in v1 — bundles consider only direct edges in the just-passed item's `Covers:` list, no chained inference.

The plan-review experience is built around user control. After the existing test-plan approval prompt, a second prompt surfaces the coverage graph: "Coverage shortcuts the plan suggests: Item 1 → covers 3, 4. Bundling these saves ~4 prompts. Approve, edit, or strip all?" If the agent's guesses look wrong, you can drop edges one at a time, add new ones (with cycle/self-reference/existence validation), or strip the whole graph in one click — that reverts the session to today's per-item behavior. Three consecutive invalid edits trigger a bail-out prompt so the loop can't trap you. The Add Item flow in Phase 2 gets a covers picker too: new items can declare what they verify before being appended.

The infrastructure scales: BATCH mode keeps index-ordered selection (no leapfrogging — that would break checkpoints), but adds in-batch cluster hints + a post-batch walk that fires bundle prompts for out-of-batch covers in item order. Phase 3 resume re-validates the persisted covers graph and demotes orphaned `PASSED_BY_COVERAGE` items if their covering item was removed externally between sessions. Phase 3 dashboard renders a COVERAGE sub-block when any bundles exist. Phase 4 report adds "Coverage savings: K items confirmed by coverage across B bundles (saved K prompts)" so you can see how much the feature actually saved per session — and a "Bundles accepted: B/O" line surfaces the inference acceptance rate as calibration data (sub-50% means heuristics need tuning).

Design pipeline: this feature went through `/plan-ceo-review` SELECTIVE EXPANSION mode (5 cherry-picks all accepted, 3-iteration spec review 5→7→9/10), `/codex review` outside-voice (13 critiques, 5 patched inline including the multi-cover provenance rule and the demotion-on-all-FAIL-paths fix), and `/plan-eng-review` (5 findings, all patched — including a "prose-as-code adaptation" pattern for the markdown skill: helper functions become named-reference patterns (`terminal-passed statuses`), cache invalidation becomes load-bearing imperative sentences with grep invariants). 27 new test invariants in `tests/skill-protocols.test.ts` lock the prompt-options table (5 sites with exact strings), the trust-posture language, the load-bearing E3 re-read sentence, the multi-cover demotion rule, the transitive-coverage non-support note, and the BATCH out-of-batch walk language. 4 new tests in `tests/parsers-pair-review-session.test.ts` lock forward-compat with `PASSED_BY_COVERAGE` and backward-compat with pre-coverage group files.

Pre-existing test failure: `tests/parsers-roadmap.test.ts:484` asserts Group 6 is "active" (uncompleted), but ROADMAP.md now lists Groups 6-11 as shipped. The assertion's premise rotted as the roadmap advanced past it. Unrelated to smart-batching; fix in a separate PR. Not blocking.
...
Closes the CEO-reviewed deferral from Track 5A. `/roadmap` now auto-offers to scaffold a project's `docs/` layout (`docs/`, `docs/designs/`, `docs/archive/`) and execute the audit's `Suggested: git mv` lines in one batch confirm when the audit reports misplaced docs or no `docs/` directory on a CLAUDE.md-onboarded project. The whole flow is skill-prose-only — no new binary subcommand, no parallel JSON channel — because `doc-type.ts` already shipped pre-quoted shell-safe suggestions and `/roadmap` already orchestrates user prompts via AskUserQuestion.

**New skill section** in `skills/roadmap.md`: "Layout Scaffolding" describes trigger detection (DOC_LOCATION non-pass OR DOC_TYPE_MISMATCH non-pass with `git mv` lines OR new `docs/ directory absent` finding), batch plan presentation with blocked-items surfaced informationally, and the per-mv execution flow. Preflight: `git rev-parse --is-inside-work-tree` distinguishes not-in-git from in-git-but-untracked so the executor picks `mv` vs `git mv` per-file via `git ls-files --error-unmatch --`. All scaffold directories validated before any moves; partial failures HALT with a summary listing completed/failed/not-attempted; no rollback (idempotent re-run handles recovery). Plain-`mv` branch has explicit `[ -e "<dst>" ]` collision precheck — `git mv` refuses to overwrite by default, but plain `mv` is silently destructive (codex catch on two design docs with same basename collapsing to one destination).

**Audit fixes** consumed by the new flow: `src/audit/checks/doc-location.ts` emits a new `docs/ directory absent` finding when `docs/` is missing AND no project doc lives at root AND CLAUDE.md is present at root or `docs/` — the CLAUDE.md gate is load-bearing (without it the finding fires on every bare repo `roadmap-audit` happens to touch, dirtying ~27 of 28 snapshot fixtures; with it, only the new `docs-dir-absent-with-claude` fixture exercises the case). The check also gains `Suggested: git mv` lines on every existing root-misplacement and docs-misplacement finding so the Layout Scaffolding move list can actually act on them — pre-Track-8A, DOC_LOCATION emitted descriptive findings only and the move list would have been empty for the most common case (TODOS.md/ROADMAP.md/PROGRESS.md in root). `taxonomy.ts` both-exist findings now name the precedence reality ("X.md exists in BOTH root and docs/ — root copy is used; docs/ copy is invisible to the audit. Reconcile manually.") so users who hit the collision case know which copy is canonical without re-reading `cli.ts:findDoc`. The DOC_LOCATION both-exist branch is deliberately NOT duplicated — TAXONOMY already owns that signal.

**doc-type.ts inbox always-block:** `DOC_TYPE_MISMATCH` findings with checkbox-density ≥ 0.50 outside `TODOS.md` are now always-blocked — the `suggestionFor` function short-circuits before destination resolution and emits `Suggested: review and move (no automated suggestion — inbox content typically wants merge, not rename)`. A checkbox-heavy markdown file outside `TODOS.md` is almost always a second inbox needing merge/import, not a rename, so `git mv` would be the wrong tool. The legacy `inboxDestination` resolver is removed as dead code; design-mismatch findings (mermaid/plantuml outside `docs/designs/`) keep the existing `git mv` suggestion behavior with `mkdir -p` prefix when the parent is missing.

**Shared infra:** `shellQuote` is hoisted from `doc-type.ts` to `src/audit/lib/shell-quote.ts` so `doc-location.ts`'s new `Suggested:` lines and `doc-type.ts`'s existing suggestions share a single quoting implementation. POSIX single-quote semantics (`'\''` close-escape-reopen for embedded quotes) plus the `--` end-of-options sentinel on `git mv` defend against malicious filenames at copy-paste time. Track 5A codex ship review caught the unquoted-shell-meta P0; Track 8A codex review caught the duplicate-implementation risk when adding the second consumer.

**Tests:** new `tests/checks-doc-location.test.ts` (6 unit tests pinning the CLAUDE.md gate's fire/no-fire cases including chezmoi-style `docsClaude && !docsDir`); `tests/checks-doc-type.test.ts` inbox describe block rewritten as 4 always-block tests + short-circuit test asserting collision detection never runs for inbox findings; `tests/skill-protocols.test.ts` gains 5 new required-token assertions locking the load-bearing strings in skills/roadmap.md (`Layout Scaffolding`, `git rev-parse --is-inside-work-tree`, `git ls-files --error-unmatch --`, the inbox always-block reason text, idempotent re-run wording). Two new snapshot fixtures: `tests/roadmap-audit/docs-dir-absent-with-claude/` exercises the CLAUDE.md-gated docs/-absent finding; `tests/roadmap-audit/taxonomy-both-exist/` exercises the precedence wording end-to-end. Existing fixtures: `doc-location-bad/expected.txt` gains 3 `Suggested:` lines (mechanical); `doc-type-mismatch/expected.txt` shows the new inbox always-block wording.

**Process notes:** Track shipped after a full CEO review (HOLD SCOPE) plus eng review plus two codex outside-voice passes. The CEO codex round reshaped the original scope (binary subcommand + JSON contract + contract test + ~150 LOC TS) into the skill-prose-only shape (~110 LOC) by reframing the problem as "consume existing audit output" rather than "invent a parallel JSON channel for findings that already render as suggestions." The eng codex round caught 5 more issues — 3 of which (silent overwrite on plain-mv, missing Suggested lines in doc-location, mkdir-prefix trigger miss) were fixed in this PR before ship; 2 (CLAUDE.md gate too broad for non-gstack repos, symlink scaffold defense-in-depth) deferred as `[review:track-8a]` TODOs.
