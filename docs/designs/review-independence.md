...
The original author logs are `author-openai.log` and `author-anthropic.log` in the probe directory. Both contain their seven-character commit IDs, but neither contains the full commit ID; the latter has no model field. They are evidence of the reported author commands, not complete per-model author proofs. Pins: OpenAI `2d31ae726cc7f7024dfc1cec5e58649d1f7dd588` (280 lines) and Anthropic `73b626703a5de0227a007438942e3423cc2021ef` (332 lines). The OpenAI log names `gpt-6-astra`. Full writer-transcript binding was not frozen, so the probe author sets remain assignment-level.
