...
### [ship] `bun run test` is ~3 min slower than design target — `audit-snapshots.test.ts` (124s) and `audit-shadow.test.ts` (69s) dominate
Surfaced during /ship + /land-and-deploy on `kbitz/track-4d-impl` (2026-05-04). The full suite via `scripts/select-tests.ts` "running all" path takes 184–325s on a 2-core MacBook (varies with system load), of which `tests/audit-snapshots.test.ts` is ~124s (14 fixtures × ~9s/fixture invoking `bin/roadmap-audit`) and `tests/audit-shadow.test.ts` is ~69s (bash-vs-bun shadow parity test). Confirmed pre-existing on `origin/main` 696ded8 (Track 4A) — same 125s/91s timings — so NOT introduced by Track 4D.
- **Symptom:** `time bun test tests/audit-snapshots.test.ts` → ~124s; `time bun test tests/audit-shadow.test.ts` → ~69s. Combined these two files take longer than the entire design-doc target (CLAUDE.md `## Testing` references a "<5s" snapshot suite target from Track 2A). Also pushes against Bash 5-min default tool timeouts, causing SIGKILL on `bun run test:full` on busy machines.
- **Repro:** From the gstack-extend repo root: `time bun test tests/audit-snapshots.test.ts` and `time bun test tests/audit-shadow.test.ts`.
- **Hypothesis (untested):** `audit-snapshots.test.ts` invokes the compiled `bin/roadmap-audit` once per fixture (14 fixtures, sequential). Each invocation pays bun startup (~50–100ms) plus the audit run; if audit itself is ~9s on these synthetic fixtures the per-invocation cost dominates. Investigate: `time` a single fixture to separate startup from per-fixture cost; check whether `bin/roadmap-audit` rebuilds implicitly per-test; consider parallelizing fixture invocations within the test (Promise.all over the 14). For `audit-shadow.test.ts`, the 69s is approximately the bash `bin/roadmap-audit` runtime — once the bash sibling retires (Track 2A's stated end-state), audit-shadow goes away naturally.
- **Effort:** S (human: ~1–2 hr / CC: ~30 min) — primarily investigation; fix may be as small as parallelizing the snapshot fixture loop.
- **Depends on:** nothing blocking. `audit-shadow.test.ts` self-resolves when bash audit retires.
