...
it; each valid explicit --start or --session-id takes precedence. Bad flag arity
skips the call. Without valid state, finish writes nothing. A successful finish
...
$HOME host skill directories. GSTACK_EXTEND_DIR is not required. CWD-relative
pointers are ignored. Existing setup wiring
already handles this binary, and upgrade invokes setup again.
...
is diagnosed by the shell guard in debug mode, or by doctor.
