...
const PORTED_CHECKS: Array<(ctx: AuditCtx) => CheckResult> = [
  runCheckVocabLint,
  runCheckStructure,
  runCheckPhases,
...
export function runAudit(ctx: AuditCtx): string {
  const out: string[] = [];
  for (const check of PORTED_CHECKS) {
...
// ─── --scan-state stub ────────────────────────────────────────────────
//
// Emits the JSON envelope documented at bash L3825-3837. PR-2 only ports
// 12 of 24 checks; un-ported signals (in_flight_groups, origin_total,
// has_zero_open_group, git_inferred_freshness) come from PR-3 checks, so
// emit zero/empty placeholders here. The shadow test does not diff
// scan-state output (it's not in PORTED_SECTIONS); bash retains coverage
// via the existing snapshot suite.
...
export function runScanState(ctx: AuditCtx, prompt: string | null): string {
    ...
    return JSON.stringify(
      {
        ...
        intents: { closure, split, track_ref: trackRef },
        signals: null,
      },
      null,
      2,
  ...
  // Mirror staleness signal (the only PR-2-ported field consumed).
  const staleness = runCheckStaleness(ctx);
  const stalenessFail = staleness.status === 'fail' ? 1 : 0;
