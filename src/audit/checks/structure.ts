/**
 * structure.ts — ROADMAP.md skeleton validation.
 *
 * Validates:
 ...
 *   - No Track has an intra-Group `_Depends on: Track NX_` — Tracks within
 *     a Group must be fully parallel-safe; sequential file work belongs in
 *     one Track or in different Groups.
 *   - No Track body contains "Ship as N PRs" / "PR1" / "PR2" / "two PRs"
 *     style language — 1 Track = 1 PR, no exceptions.
...
export function runCheckStructure(ctx: AuditCtx): CheckResult {
  ...
  // Intra-Group _Depends on: Track NX_ — banned.
