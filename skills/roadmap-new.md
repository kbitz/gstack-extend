...
description: |
  ...
  Ships alongside the v1 `/roadmap` skill while the model is being
  validated. Once proven, this will replace `/roadmap` and this skill name
  will be retired. Spec: `docs/designs/roadmap-v2-state-model.md`.
  Use when asked to "regenerate the roadmap", "use the new roadmap model",
  "restructure with state sections", or to try the v2 plan model. For the
  existing v1 surgical-reassessment workflow, use `/roadmap` instead.
allowed-tools:
  - Bash
  - Read
  - Write
...
`/roadmap-new` ships alongside the existing `/roadmap` skill while the v2
model is being validated. Both can be used on the same project: the audit
auto-detects v1 vs v2 grammar and only enforces v2-specific rules
(intra-Group `_Depends on:_` ban, "N PRs" language ban, Hotfix invariants)
when v2 grammar is detected. Once the v2 model is proven, `/roadmap-new`
will replace `/roadmap`.
