...
if [ -n "$_GE_BIN" ]; then
  "$_GE_BIN" start --skill "extend:review-apparatus" || true
...
find "$(session_dir pair-review)/branches" -maxdepth 2 -name report.md 2>/dev/null
```
...
if [ -n "$_GE_BIN" ]; then
  "$_GE_BIN" finish --skill "extend:review-apparatus" --outcome unknown || true
elif [ "${GSTACK_EXTEND_TELEMETRY_DEBUG:-}" = "1" ]; then
  echo 'telemetry skipped: gstack-extend-telemetry unresolvable or stale. Fix: re-run ./setup. See docs/telemetry.md.' >&2
fi
true
